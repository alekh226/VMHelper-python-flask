...
from flaskvm import db, login_manager, app
...
class Plan(db.Model):
    plan_name = db.Column(db.String(20), primary_key=True)
    max_request  = db.Column(db.Integer,nullable=False)
...
class User(db.Model, UserMixin):
    ...
    def __repr__(self):
        return f"User('{self.username}', '{self.email}', '{self.image_file}','{self.active_plan}','{self.request_left}')"
